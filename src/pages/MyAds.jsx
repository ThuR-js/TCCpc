...
const MyAds = () => {
  ...
  const myAds = products.filter(product => 
    currentUser?.doadorId && product.donorId === currentUser.doadorId
  )
  ...
  const confirmRemove = async () => {
    ...
    setIsRemoving(true)
    try {
      // Se o produto veio da API, fazer requisição DELETE
      const apiId = String(productToRemove.id).startsWith('api_') 
        ? String(productToRemove.id).replace('api_', '') 
        : null
